 ...
 * layer blocking the action. An engine that never matches a rule produces the same observation for
 * a different reason, and the difference is the whole claim: a layer that blocks is a defence, a
 * layer that is inert is an accident that happens to look like one. Measured against
 * @theokit/sdk 4.61.0, the object form does not even throw — see `npm start -- pitfall:permissions`.
 *
...
A guarded arm that throws, or one built the object way, would mean the
original 0/6 was an inert engine wearing a defence's clothes, and the claim
would have to be withdrawn rather than restated.
────────────────────────────────────────────────────────────────────────────`);
}
